...
(function(){
  ...
  function appendPosts(stok){
    ...
    fragment.appendChild(error('Далее информация теряет актуальность.')[0]);
    fragment.appendChild(warning('Борманду надо запилить заголовок "Access-Control-Allow-Origin: http://govnokod.ru", иначе это всё - питушня.')[0]);
    
    if(i >= stok.comments.length)
      fragment.appendChild(warning('Возможно, сток порван.'));
